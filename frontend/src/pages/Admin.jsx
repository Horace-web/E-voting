    ...
    const loadDashboardData = async () => {
      try {
        ...
        try {
          ...
          elections = normalizeApiResponse(electionsResponse);
          setRecentElections(elections.slice(0, 3));
        } catch (error) {
          console.warn(
        ...
        try {
          ...
          totalVoters = userData?.total || normalizeApiResponse(usersResponse).length || 0;
        } catch (error) {
          console.warn(
        ...
        const activeElections = elections.filter(
          (e) => e.statut === "En cours" || e.statut === "en_cours" || e.statut === "Publiée"
        ).length;

        const totalVotesCount = elections.reduce((acc, e) => acc + (parseInt(e.votes_count) || 0), 0);
        ...
        setStats({
          ...
          participationRate: totalVoters > 0 
            ? Math.round((totalVotesCount / totalVoters) * 100) 
            : 0,
          pendingValidations: 0,
        });
      } catch (error) {
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div>
            ...
            <h1 className="text-2xl lg:text-3xl font-bold text-white mb-2">
              Bonjour, {currentUser.prenom || currentUser.firstName || "Admin"} !!
            </h1>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {recentElections.map((election) => {
              const statusConfig = getStatusConfig(election.statut || election.status);
              const votesCount = parseInt(election.votes_count) || parseInt(election.votes) || 0;
              const totalVotersCount = stats.totalVoters || 1;
              const progress = Math.min((votesCount / totalVotersCount) * 100, 100);
                ...
                <div key={election.id} className="p-6 hover:bg-gray-50/50 transition-colors group">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-[#1e3a5f] transition-colors">
                        {election.titre || election.title}
                      </h3>
                      <div className="flex items-center gap-4 text-sm text-gray-500">
                        <span className="flex items-center gap-1.5">
                          <Calendar size={14} className="text-gray-400" />
                          {election.date_debut ? new Date(election.date_debut).toLocaleDateString("fr-FR", {
                            day: "numeric",
                            month: "short",
                          }) : "N/A"}{" "}
                          -{" "}
                          {election.date_fin ? new Date(election.date_fin).toLocaleDateString("fr-FR", {
                            day: "numeric",
                            month: "short",
                            year: "numeric",
                          }) : "N/A"}
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${statusConfig.dot} ${(election.statut === "En cours" || election.statut === "Publiée") ? "animate-pulse" : ""}`}
                      ></span>
                      {election.statut || election.status}
                    </span>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right min-w-[80px]">
                      <span className="text-sm font-bold text-gray-800">{votesCount}</span>
                      <span className="text-sm text-gray-400">/{totalVotersCount}</span>
                    </div>
                  </div>
