...
function Resultats() {
  ...
  // Charger les élections avec résultats depuis l'API
  useEffect(() => {
    loadElections();
  }, []);
  ...
  const loadElections = async () => {
    try {
      ...
      const electionsData = Array.isArray(response) ? response : (response.data || []);
  ...
  const tauxParticipation = selectedElection ? (
  ...
  const gagnant = selectedElection?.candidats?.length > 0 
    ...
    : null;
  const totalVotes = selectedElection?.total_votes || 0;
  ...
  const togglePublication = (electionId) => {
    ...
    alert(
      `Publication ${election.publie ? "retirée" : "activée"} pour cette élection`
    );
  };
  ...
  const exportPDF = () => {
    if (!selectedElection) {
      alert("Aucune élection sélectionnée");
      return;
    }
  ...
  const exportCSV = () => {
    ...
    const csv = [
      `Résultats: ${selectedElection?.titre || 'Sans titre'}`,
      `Date de clôture: ${selectedElection?.date_cloture ? new Date(selectedElection.date_cloture).toLocaleDateString("fr-FR") : 'Non définie'}`,
    ...
    a.href = url;
    a.download = `resultats_${selectedElection?.titre?.replace(/\s+/g, "_") || 'sans_titre'}.csv`;
    a.click();
  };
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-2xl font-bold text-blue-600">
                      {selectedElection?.total_electeurs?.toLocaleString() || "0"}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      {selectedElection?.total_votes?.toLocaleString() || "0"}
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-sm p-5 border-l-4 border-amber-500 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-lg font-bold text-amber-600 truncate" title={gagnant?.nom || 'En attente'}>
                      {gagnant?.nom || 'En attente'}
                    </p>
                  </div>
