...
const ElecteurLayout = ({ children }) => {
  ...
  const menuItems = [
    { id: "dashboard", label: "Tableau de bord", icon: LayoutDashboard, path: "/electeur" },
    { id: "vote", label: "Voter", icon: Vote, path: "/electeur/vote" },
    { id: "results", label: "Résultats", icon: BarChart3, path: "/electeur/resultats" },
    { id: "history", label: "Mes participations", icon: History, path: "/electeur/participations" },
  ];
  ...
  const isActive = (path) => {
    if (path === "/electeur") {
      return location.pathname === "/electeur";
    }
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-xs text-amber-400/90 font-medium">Espace Électeur</p>
            </div>
          </Link>
          <button
      ...
      <div className="flex-1 flex flex-col min-h-screen">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-[#1e3a5f]">
                  {menuItems.find((item) => isActive(item.path))?.label || "Tableau de bord"}
                </h2>
                <p className="text-sm text-gray-500">Bienvenue sur votre espace de vote sécurisé</p>
              </div>
            </div>
        ...
        <footer className="bg-white border-t border-gray-100 px-4 lg:px-8 py-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-2 text-sm text-gray-500">
            <p>© 2026 E-Vote. Plateforme de vote électronique sécurisée.</p>
            <p className="flex items-center gap-1">
              <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
              Système sécurisé et chiffré
            </p>
          </div>
        </footer>
      </div>
