...
try {
    ...
    $response = Invoke-RestMethod -Uri "$BASE_URL/users" -Method POST -ContentType "application/json" -Headers @{
...
try {
    $response = Invoke-RestMethod -Uri "$BASE_URL/auth/me" -Method GET -Headers @{
...
try {
    $response = Invoke-RestMethod -Uri "$BASE_URL/auth/logout" -Method POST -Headers @{
        Authorization = "Bearer $USER_TOKEN"
    } -SkipHttpErrorCheck
...
Write-Host "👤 Rôle: VOTER (Électeur)" -ForegroundColor Cyan
